...
public class VolumeDVR : MonoBehaviour
{
    ...
    private Texture2D tfTexCurrent;    // celle qu'on envoie au shader
    ...
    void Start()
    {
        ...
        // build textures (fills volumeTexture + tfLUTTextureSoft + tfLUTTextureHard)
        VRDFLoader.BuildUnityTextures(_data);

        volumeTex = _data.volumeTexture;

        // choisir la bonne LUT initiale
        tfTexCurrent = useHardTF ? _data.tfLUTTextureHard
                                 : _data.tfLUTTextureSoft;
        ...
        if (verboseDebug)
        {
            ...
            string tfDescSoft = _data.tfLUTTextureSoft ? (_data.tfLUTTextureSoft.width + "x" + _data.tfLUTTextureSoft.height + " soft") : "null";
            string tfDescHard = _data.tfLUTTextureHard ? (_data.tfLUTTextureHard.width + "x" + _data.tfLUTTextureHard.height + " hard") : "null";
            Debug.Log($"[VolumeDVR] TF soft={tfDescSoft} hard={tfDescHard}, usingHard={useHardTF}");
        }
    }

    /// <summary>
    /// Permet de changer à chaud entre LUT douce et LUT dure.
    /// Appelle ça depuis un bouton UI par ex.
    /// </summary>
    public void SetTFMode(bool hard)
    {
        useHardTF = hard;

        // choisir la LUT correspondante
        tfTexCurrent = useHardTF ? _data.tfLUTTextureHard
                                 : _data.tfLUTTextureSoft;

        // renvoyer au material
        if (volumeMaterial != null && tfTexCurrent != null)
        {
            volumeMaterial.SetTexture("_TFTex", tfTexCurrent);
        }

        // Rebuild labelInfos (les couleurs affichées dans le menu changent aussi)
        BuildLabelInfosFromData();
    }
    ...
    private void ApplyToMaterial(VRDFVolumeData d)
    {
        ...
        // ensure mesh renderer has that material
    ...
    /// <summary>
    /// Reconstruit la liste labelInfos telle qu'elle doit apparaître dans ton menu UI.
    /// On lit les pixels depuis tfTexCurrent (donc dépend du mode soft/hard).
    /// </summary>
    private void BuildLabelInfosFromData()
    {
        labelInfos.Clear();

        if (tfTexCurrent == null || _data == null || _data.tf == null)
            return;

        // lut actuelle
        Color[] lutPixels = tfTexCurrent.GetPixels();
        int lutLen = lutPixels.Length;

        // map label->nom humain
        ...
        if (_data.tf.entries != null)
        {
            foreach (var e in _data.tf.entries)
            {
                ...
                {
                    string nice = string.IsNullOrEmpty(e.name) ? ("Label " + lbl) : e.name;
                    names[lbl] = nice;
                }
            }
        }
    ...
    private void FitVolumeScaleFromSpacing(VRDFVolumeData data)
    {
        ...
        // position: ici on laisse 0,0,0 local; tu peux remettre center patient si tu veux
    }

    //===== les fonctions runtime de masquage / opacité / tint ne changent pas =====
